...
称公式的有穷序列$A_1, A_2, ..., A_n$是一个*形式证明*，如果对任意$i$，$1\leq i \geq n$，都有下列之一成立：
  ...
  3. 存在$j < i$使得$A_i$是$A_j$关于规则 CGen 的直接后承，或
  4. 存在$j < i$使得$A_i$是$A_j$关于规则 PEx 的直接后承。
...
### 谓词演算的可靠性定理
引理：谓词演算的每一个公理都是永真的。
**可靠性定理**：谓词演算的所有定理都是永真的

### 推演定理
命题演算中的推演定理不能直接推广到谓词演算中。

#### 依赖性与变化性
**依赖性**定义：设$\Gamma$是由若干公式组成的集合，公式序列$A_1, ..., A_n$是一个由$\Gamma$中公式出发的形式推演，D 是$\Gamma$中的一个公式。说在上述推演中$A_i$依赖于 D，若下列之一成立：
  1. $A_i$就是$D$
  2. 存在$j_1, j_2 < i$使得$A_i$是$A_{j_1}$和$A_{j_2}$关于分离规则的直接后承，且$A_{j_1}$和$A_{j_2}$中至少有一个依赖于$D$
  3. 存在$j < i$使得$A_i$是$A_j$关于 CGen 规则的直接后承，且$A_j$依赖于$D$
  4. 存在$j < i$使得$A_i$是$A_j$关于 PEx 规则的直接后承，且$A_j$依赖于$D$

**变化性**定义：就公式 D 而言，变元 x 在所给的推演中是变化的，若下列同时成立：
  1. x 在 D 中自由出现
  2. 在推演过程中对依赖于 D 的公式就 x 应用了 CGen 或PEx

在给定的推演中 x 对 D 而言是不变化的，则称 x 对 D 而言在推演中是固定的

#### 谓词演算的推演定理
定理：对谓词演算而言，若$\Gamma, A\vdash B$，且在该推演中所有自由变元对 A 来说是固定的，那么$\Gamma\vdash A\to B$
