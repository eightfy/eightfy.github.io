...
重言式（tautology），永真；自相矛盾，永假
真值表检测命题：不会得到真前提、假结论。将每个结论设为 1 而结论为 0，接着计算每个字母对应真值，若出现矛盾则论证有效。
...
### 量化语言
quantification language：包括命题逻辑的字母表、公式、推理规则和证明，增加三类新的字母类型：小写字母、“$\forall$”、“$\exists$”
> 使用大写字母表示**通项**，描述或指定一个范畴（category）
>
> 使用小写字母表示**单项**，指出特定的人或事物（专有名词）

一个**量词**（quantifier）是一个形如“$(\forall x)$”（全称量词）或“$(\exists x)$”（存在量词）的字符序列，任何变元都可取代其中的 x。
在命题规则上新增两条新规则：
...
### 关系及其证明
第四个构造量化公式的规则引入关系：
> 大写字母后跟一对括号，括号中有两个以上小写字母，得到的是一个公式

丘奇定理：对关系论证不存在总可以给出证明或反驳的算法
